...
impl<'a> TranslationUnit<'a> {
    pub fn add_preprocessor_define(&mut self, key: &str, value: &str) -> &mut Self {
        ...
        vcall!(
            self.request,
            addTranslationUnitPreprocessorDefine(self.index, key.as_ptr(), value.as_ptr())
        );
        self
    }
    ...
    pub fn add_source_file(&mut self, path: impl AsRef<Path>) -> &mut Self {
        ...
        vcall!(
            self.request,
            addTranslationUnitSourceFile(self.index, path.as_ptr())
        );
        self
    }
    ...
    pub fn add_source_string(&mut self, path: impl AsRef<Path>, source: &str) -> &mut Self {
        ...
        vcall!(
            self.request,
            addTranslationUnitSourceString(self.index, path.as_ptr(), source.as_ptr())
        );
        self
    }
    ...
    pub fn add_entry_point(&mut self, name: &str, stage: Stage) -> EntryPointIndex {
        ...
        let index = vcall!(
            self.request,
            addEntryPoint(self.index, name.as_ptr(), stage)
        );
